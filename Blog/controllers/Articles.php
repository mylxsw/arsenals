...
class Articles extends CoreController {
	...
	public function lists(Input $input, $category_id){
		...
		$this->assign('breadcrumbs', array('首页'=>'', $category['name']=> "category/{$category['id']}.html" ));
	...
	public function show($id){
		...
		$this->assign('id', $id);
        $this->assign('breadcrumbs', count($article['cate']) > 0 ? array('首页'=>'', $article['cate'][0]['name']=> "category/{$article['cate'][0]['id']}.html" , $article['title'] => $article['id'] ) : '');
