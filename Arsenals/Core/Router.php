...
{
    private static $_router_defined = [];
    private static $_router_data = [];
     ...
     * @var array
     */
    private static $_regexp_replace_vars = [
        ':num'                => '[0-9]+',
        ':any'                => '.+',
    ];
    ...
    public static function map($url, $route)
    {
        ...
        if (preg_match($key, $_instance->_path_info)) {
            ...
            self::$_stop = true;
            // 完成路由调度并返回相应的视图对象
            $_instance->dispatch_map([$key, $route], $args);
        }
    }
    ...
    public function dispatch_map($callback_funcs, $args)
    {
        ...
        } elseif (is_callable($controller_func)) {// 如果可执行，则为回调函数
            // 反射获取参数对象
