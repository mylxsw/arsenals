...
class Authencation
{
    ...
    public function __construct($auth = [])
    {
        ...
        if (array_key_exists('anonymous', $auth['rules'])) {
            $this->_rules['anonymous'] = array_merge($this->_rules['anonymous'], $auth['rules']['anonymous']);
    ...
    private function _mergeInheritPermission($rolename, $type = 'access')
    {
        ...
        // 如果存在继承角色，则递归进行合并
        return array_merge($this->_mergeInheritPermission($inherit, $type), $this->_rules[$rolename][$type]);
    }
