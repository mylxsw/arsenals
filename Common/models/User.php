...
class User extends Model
{
    ...
    {
        return $this->load(['username' => $username, 'password' => sha1($password), 'isvalid' => 1]);
    }
    ...
    {
        $this->update(['password' => sha1($password)], ['username' => $username]);
    }
    ...
    {
        $user = $this->load(['username' => $current_user]);
        if (is_null($user)) {
    ...
    public function addUser(array $data)
    {
        ...
        // 检查确保用户唯一
        $user = $this->load(['username' => $entity['username']]);
        if (!is_null($user)) {
